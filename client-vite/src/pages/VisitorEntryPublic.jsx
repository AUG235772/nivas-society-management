...
import { Shield, UserPlus, Calendar, Clock, Car, Trash2 } from 'lucide-react';

const MyVisitors = () => {
  const { user } = useContext(AuthContext);
  const [visitors, setVisitors] = useState([]);
  const [form, setForm] = useState({ 
    name: '', phone: '', purpose: '', vehicleNumber: '', duration: '1' 
  });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <form onSubmit={handlePreApprove} className="space-y-4">
              <div>
                ...
                <input 
                  ...
                  required 
                  className="w-full p-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-800 font-bold focus:outline-none focus:ring-2 focus:ring-indigo-500" 
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                   ...
                   <input 
                     ...
                     className="w-full p-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-800 font-bold focus:outline-none focus:ring-2 focus:ring-indigo-500 uppercase" 
                   />
                </div>
                <div>
                   ...
                   <select 
                     ...
                     value={form.duration} 
                     onChange={handleChange} 
                     className="w-full p-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-800 font-bold focus:outline-none focus:ring-2 focus:ring-indigo-500 cursor-pointer"
                   >
              ...
              <button 
                disabled={submitting} 
                className="w-full py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-lg mt-2 transition-all active:scale-95"
              >
        ...
        <div className="lg:col-span-2 space-y-6">
           <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
             <Calendar size={20} className="text-indigo-600"/> Visitor History
           </h2>
           ...
           {loading ? <p className="text-center text-slate-400 py-10">Loading history...</p> : 
            Object.keys(groupedVisitors).length > 0 ? (
                         ...
                         {logs.map(v => (
                           <tr key={v._id} className="hover:bg-slate-50 transition-colors">
                             <td className="px-6 py-3">
                               ...
                               {v.vehicleNumber && (
                                 <span className="inline-flex items-center gap-1 mt-1 px-1.5 py-0.5 bg-slate-100 text-slate-600 text-[10px] font-bold rounded">
                                   <Car size={10}/> {v.vehicleNumber}
                                 </span>
                               )}
                             </td>
                             <td className="px-6 py-3">
                               <p className="text-xs text-slate-500 font-medium">{v.purpose}</p>
                             </td>
                             <td className="px-6 py-3 text-xs font-medium text-slate-600">
                               <div className="flex items-center gap-1">
                                 <Clock size={12} className="text-emerald-500"/> In: {new Date(v.entryTime).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}
                               </div>
                               <div className="flex items-center gap-1 mt-1">
                                 <Clock size={12} className="text-rose-500"/> Out: {v.exitTime ? new Date(v.exitTime).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'}) : '-'}
                               </div>
                             </td>
                             <td className="px-6 py-3">
                               {v.status === 'Inside' 
                                 ? <span className="text-[10px] font-bold bg-emerald-100 text-emerald-700 px-2 py-1 rounded-full">INSIDE</span>
                                 : (v.status === 'Expected' 
                                     ? <span className="text-[10px] font-bold bg-blue-100 text-blue-700 px-2 py-1 rounded-full">EXPECTED</span>
                                     : <span className="text-[10px] font-bold bg-slate-100 text-slate-500 px-2 py-1 rounded-full">EXITED</span>
                                   )
                               }
                             </td>
                           </tr>
              ...
              <div className="p-10 text-center bg-white rounded-2xl border border-dashed border-slate-200 text-slate-400">
                No visitors found for your flat.
              </div>
            )
           }
        </div>
